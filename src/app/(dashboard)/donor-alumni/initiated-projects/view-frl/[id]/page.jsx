...
const ProgramReportsPage = () => {
  ...
  const programName = programNames[programId] || "Project";
  // Sample reports data
  const reports = [
    {
      id: 1,
      ...
      frl: "https://publicbucket-development.zohostratus.in/pdf/FRL%20Format.pdf",
    },
    {
      id: 2,
      referenceNo: "87790",
  ...
  const handleDownloadReport = (pdfUrl, programName) => {
    const link = document.createElement("a");
    link.href = pdfUrl;
    link.download = `${programName.replace(/\s+/g, "_")}_Report.pdf`;
    link.target = "_blank";
    ...
    <div className="w-full h-full">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          <div>
            <h1 className="sm:text-xl text-lg font-bold text-brand-500">
              {programName}
            </h1>
          </div>
        </div>
        <button
          onClick={() => router.back()}
          className="text-brand-600 dark:text-brand-400 hover:underline text-sm sm:text-base"
        >
          <span className="sm:inline hidden">← Back to Initiated Projects</span>
          <span className="sm:hidden inline">← Back</span>
        </button>
      </div>

      {/* Reports Table - Same for both mobile and desktop */}
            ...
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05] bg-gray-50 dark:bg-gray-800/50">
                <TableRow>
                  <TableCell
                    isHeader
                    className="px-3 py-3 font-semibold text-gray-700 dark:text-gray-300 text-start text-sm whitespace-nowrap sm:hidden block"
                  >
                    FRL
                  </TableCell>
                  <TableCell
                    isHeader
                    className="px-3 py-3 font-semibold text-gray-700 dark:text-gray-300 text-start text-sm whitespace-nowrap"
                  >
                    Reference No.
                  </TableCell>
                  <TableCell
                  ...
                  <TableCell
                    isHeader
                    className="px-3 py-3 font-semibold text-gray-700 dark:text-gray-300 text-start text-sm whitespace-nowrap min-w-[200px]"
                  >
                    Title of the Project
                  </TableCell>
                  <TableCell
                    isHeader
                    className="px-3 py-3 font-semibold text-gray-700 dark:text-gray-300 text-start text-sm whitespace-nowrap sm:block hidden"
                  >
                    FRL
                  </TableCell>
                </TableRow>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {reports.map((report) => (
                  ...
                  >
                    <TableCell className="px-3 py-3 text-start text-theme-sm sm:hidden block">
                      <Button
                    ...
                    <TableCell className="px-3 py-3 text-start text-theme-sm text-gray-600 dark:text-gray-300 font-medium whitespace-nowrap">
                      {report.referenceNo}
                    </TableCell>
                    <TableCell className="px-3 py-3 text-start text-theme-sm text-gray-600 dark:text-gray-300 whitespace-nowrap">
                      {report.date}
                    </TableCell>
                    <TableCell className="px-3 py-3 text-start text-theme-sm text-gray-600 dark:text-gray-300 min-w-[200px]">
                      <span className="line-clamp-2">{report.title}</span>
                    </TableCell>
                    <TableCell className="px-3 py-3 text-start text-theme-sm sm:block hidden">
                      <Button
                        onClick={() => openPdfViewer(report.frl)}
                        // size="sm"
                        // variant="outline"
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 p-4 sm:p-6 border-t border-gray-200 dark:border-gray-700">
              <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 text-center sm:text-left">
                {isPdfLoading ? (
                  ...
                  `Viewing: ${programName} - FRL Document`
                ) : (
                  "Ready to view"
                )}
              </div>
